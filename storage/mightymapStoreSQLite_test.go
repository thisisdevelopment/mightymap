...
func TestMightyMapSQLiteStorageOptions(t *testing.T) {
	// Test with file-based storage
	t.Run("File-based storage", func(t *testing.T) {
		store := NewMightyMapSQLiteStorage[string, int](
			WithSQLiteDBPath("test.db"),
			WithSQLiteJournalMode("WAL"),
			WithSQLiteSyncMode("NORMAL"),
			WithSQLiteMaxOpenConns(10),
			WithSQLiteMaxIdleConns(5),
			WithSQLiteCountCacheDuration(5*time.Second),
			WithSQLitePragma("cache_size", "-1000"),
		)
		defer store.Close(context.Background())
	...
	t.Run("In-memory storage", func(t *testing.T) {
		store := NewMightyMapSQLiteStorage[string, int](
			WithSQLiteInMemory(),
			WithSQLiteTableName("custom_table"),
		)
		defer store.Close(context.Background())

		ctx := context.Background()
		store.Store(ctx, "key2", 2)
		value, ok := store.Load(ctx, "key2")
		if !ok || value != 2 {
...
func TestMightyMapSQLiteStorageConcurrent(t *testing.T) {
	store := NewMightyMapSQLiteStorage[string, int]()
	defer store.Close(context.Background())
...
func TestMightyMapSQLiteStorageEdgeCases(t *testing.T) {
	store := NewMightyMapSQLiteStorage[string, int]()
	defer store.Close(context.Background())
